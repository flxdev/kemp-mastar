$(document).ready(function () {
	...
	function resetTabs() {
		$(".tab__list").each(function(){
			...
			setTimeout(function(){
				$('.tab__list').find('li').first().addClass('is-active').siblings().removeClass('is-active');
				$('.tab__menu-container').find('.tab-item').first().find('label:first-child input').trigger('click')//.prop('checked', true);
			}, 200);
			tabs();
		});
	};
	...
	function active(){
		var item = $('.js-day'),
			tab_link = $('.tab__list').find('a');

		//item.addClass('no-active');
		tab_link.on('click', function(){
			var this_ = $(this),
				parent = this_.parents('.tab__menu'),
				linkPers = this_.data('person'),
				dayVal;
			// setTimeout(function(){
			// 	dayVal = parent.find('.visible').find('input').prop('checked', true).data('num');
			// 		console.log(linkPers, dayVal)
			// },10)
				

			
			// if(dayVal === '3') {
			// 	this_.parents('.card_cover').find('.js-day').addClass('no-active');
			// } else {
			// 	this_.parents('.card_cover').find('.js-day').removeClass('no-active');
			// }
		});
	};
	active();
	...
	(function(){
		$('.show-more a').on('click', function(event){
			var this_ = $(this),
				content = this_.parent().prev('.container'),
				linkText = this_.text();

			if (!content.hasClass('opens')) {
				content.addClass('opens');
				linkText = 'Свернуть все вопросы и ответы';
				content.switchClass('hideContent', 'showContent', 400);
			} else {
				content.removeClass('opens');
				linkText = 'Показать все вопросы и ответы';
